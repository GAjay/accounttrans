...
<html>
	...
	<body style="padding:2%"><br><BR>
		...
		<form action="push/home_value.php?user=<?php echo $user;?>&perm=<?php echo $perm;?>&pakka=challan" method="post" onkeypress="return event.keyCode != 13;">
			<?php
		...
		<br><BR><br>
			<table id="main_table">
				...
				<?php
					$sql = ("SELECT *, DATE_FORMAT(`dateofdeparture`,'%d-%m-%Y') as `dateofdeparture` FROM `challan` WHERE `paid`=0 AND partyname='1' AND `is_pakka`=1");
					$result = $db->query($sql) or die("Sql Error :" . $db->error);
					$count = 0;
					while($row = mysqli_fetch_array($result)){
						echo '<tr class="j">';
